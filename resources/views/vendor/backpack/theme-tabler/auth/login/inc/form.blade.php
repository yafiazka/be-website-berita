...
<form method="POST" action="{{ route('backpack.auth.login') }}" autocomplete="off" novalidate="">
    ...
    <div class="mb-3">
        ...
        <input autofocus tabindex="1" type="text" name="{{ $username }}" value="{{ old($username) }}" id="{{ $username }}" placeholder="Masukkan email atau username" class="form-control {{ $errors->has($username) ? 'is-invalid' : '' }}">
    ...
    <div class="mb-2">
        ...
        <div class="input-group input-group-flat password-visibility-toggler">
            <input tabindex="2" type="password" name="password" id="password" placeholder="Masukkan kata sandi" class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" value="">
